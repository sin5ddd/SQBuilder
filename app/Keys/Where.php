<?php
	...
	class Where extends ISQLKey {
		...
		static public function between(string $key, array $values): string {
			...
			$values = self::check_str($values); // DateTimeの場合もあるため、文字列エスケープする
		...
		static private function check_str($value){
			if(is_string($value)){
				$value="'$value'";
			}
			if(is_array($value)){
		...
		static private function check_num($value){
			if(is_string($value)){
